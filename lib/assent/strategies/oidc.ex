defmodule Assent.Strategy.OIDC do
  ...
  ## Configuration
    ...
    - `:nonce` - The nonce to pass in `authorize_url/1` and validated in
      `callback/3`, optional

      The nonce MUST be session based and unguessable. A the cryptographic hash
      of a cryptographically random value could be stored in a HttpOnly session
      cookie.
  ...
  @doc """
  Generates a authorization URL for request phase.
  ...
  defp validate_nonce(%{claims: %{"nonce" => nonce}}, config) do
    case Config.get(config, :nonce, nil) do
      nil    -> {:error, "`nonce` included in ID Token but doesn't exist in client configuration"}
      ^nonce -> :ok
      _      -> {:error, "Invalid `nonce` included in ID Token"}
    end
  end
  defp validate_nonce(_jwt, config) do
    case Config.fetch(config, :nonce) do
      {:error, _} -> :ok
      {:ok, _}    -> {:error, "`nonce` is not included in ID Token"}
    end
  end
